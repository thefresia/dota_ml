...
class OpendotaApi:
    ...
    def printProgressBar (self, iteration, total, prefix = '', suffix = '', decimals = 1, length = 100, fill = ' '):
        ...
        print("\n%s |%s| %s%% %s %s/%s good: %s 404: %s 429: %s" % (prefix, bar, percent, suffix, iteration, total, self.good, self.code404, self.code429), end="\n")
        if iteration == total: 
            print()
    ...
    def proceedMatches(self):
        db = MongoDbConnection.getCollection("dota_ml")
        col = db["data"]
...
caller = OpendotaApi()
caller.opendota(50000, 4895800000)
