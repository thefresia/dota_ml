...
def train_hero():
    ...
    if (count > col.count()*10):
        exit
    data = np.empty((count,13))
    original={'heroNames': [None]*200, 'heroes': [None]*count,'tsne_transformed': [np.zeros(3) for i in repeat(None, 200)]}
    ...
    for x in con["data"].find():
        ...
        for player in x["players"]:
            ...
            original["heroes"][i] = {'name' :original["heroNames"][player["hero_id"]], 'id':player["hero_id"]}
            i+=1
            if i == count:
                break
        if i == count:
    ...
    for i in range(len(res)):
        if res[i][2] != 0:
            plt.text(res[i][0], res[i][1], original["heroNames"][i])
    plt.xlabel("t-SNE feature 0")
    plt.xlabel("t-SNE feature 1")
    print(1)
